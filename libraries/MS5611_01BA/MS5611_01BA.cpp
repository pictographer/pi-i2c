...
MS5611_01BA::MS5611_01BA( I2C *i2cInterfaceIn )
...
uint32_t MS5611_01BA::GetResultCount( EResult resultTypeIn )
{
    return m_results.GetResultCount( resultTypeIn );
}

void MS5611_01BA::ClearResultCount( EResult resultTypeIn )
{
    m_results.ClearResult( resultTypeIn );
}

void MS5611_01BA::Disable()
{
...
bool MS5611_01BA::IsEnabled()
{
    return m_enabled;
}

bool MS5611_01BA::GetLock()
{
    return m_pI2C->LockAddress( m_address );
}

void MS5611_01BA::FreeLock()
{
    m_pI2C->FreeAddress( m_address );
}

EResult MS5611_01BA::SetOversampleRate( EOversampleRate rateIn )
{
...
EResult MS5611_01BA::SetWaterType( EWaterType typeIn )
{
...
uint32_t MS5611_01BA::GetUpdatePeriod()
{
...
void MS5611_01BA::Transition( EState stateIn )
{
    // Set new state
    m_state = stateIn;
}

void MS5611_01BA::DelayedTransition( EState nextState, uint32_t millisIn )
{
    // Set delay info
...
EResult MS5611_01BA::Cmd_ReadCalibrationData()
{
    uint8_t coeffs[ 2 ];
...
EResult MS5611_01BA::Cmd_StartPresConversion()
{
...
EResult MS5611_01BA::Cmd_StartTempConversion()
{
...
EResult MS5611_01BA::Cmd_ReadPressure()
{
    uint8_t bytes[ 3 ];
...
EResult MS5611_01BA::Cmd_ReadTemperature()
{
    uint8_t bytes[ 3 ];
...
uint8_t MS5611_01BA::CalculateCRC4()
{
...
void MS5611_01BA::ProcessData()
{
	...
	{
	    // Temps < 20C
		m_Ti      = ( ( int64_t )m_dT * m_dT ) / POW_2_31;
		m_OFFi    = 5 * ( ( m_TEMP - 2000 ) * ( m_TEMP - 2000 ) ) / 2 ;
		m_SENSi   = 5 * ( ( m_TEMP - 2000 ) * ( m_TEMP - 2000 ) ) / 4 ;
    	...
    	if( m_TEMP < -1500 )
    	{
    		...
    		m_SENSi   = m_SENSi + 11 * ( ( m_TEMP + 1500 ) * ( m_TEMP + 1500 ) ) / 2;
    	}
	}
	else
	{
...
// I2C call wrappers
i2c::EI2CResult MS5611_01BA::WriteByte( uint8_t registerIn )
{
    return m_pI2C->WriteByte( m_address, registerIn );
}

i2c::EI2CResult MS5611_01BA::ReadRegisterBytes( uint8_t registerIn, uint8_t *dataOut, uint8_t numBytesIn )
{
